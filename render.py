...
def render_set(dataset : ModelParams, name, iteration, views, gaussians, pipeline, background, render_mesh):
    ...
    for idx, view in enumerate(tqdm(views_loader, desc="Rendering progress")):
        if gaussians.binding != None:
            ...
            if dataset.dynamic != 'None':
                gaussians.update_offset()
          
        rendering = render(view, gaussians, pipeline, background)["render"]
        runtimes.append((datetime.now() - cur_time).total_seconds()) 
    ...
    average_fps = 1 / average_runtime
    print('average rendering time per frame is', average_runtime)
    print('average rendering fps is', average_fps)
    with open('/data1/wangyating/gsavatar_ours_v1/output/fps.txt', 'a') as file:
        file.write(dataset.model_path + ' ' + str(average_fps) + '\n')
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_val : bool, skip_test : bool, render_mesh: bool):
    with torch.no_grad():
        if dataset.bind_to_mesh:
            ...
            gaussians = FlameGaussianModel(dataset.source_path, dataset.sh_degree, static=dataset.static, dynamic=dataset.dynamic, not_finetune_flame_params = True)
        else:
