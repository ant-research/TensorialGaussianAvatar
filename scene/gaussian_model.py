...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, static: str, dynamic: str):
        ...
        self._opacity_cano = torch.empty(0) 
        self._opacity_offset = torch.empty(0)
        ...
        self.face_center_cano = None
        self.face_scaling_cano = None
        self.face_orien_mat_cano = None
    ...
    def capture(self):
        ...
        if self.dynamic == 'FeatureLine':
            # cap = cap + (self.feat_line_face.state_dict(), self.feat_line_head.state_dict(),)
            cap = cap + (self.feat_line.state_dict(), )
        if self.static == 'None':
            cap = cap + (self._features_dc, self._features_rest,)
        elif self.static == 'triplane':
            cap = cap + (self.triplane.state_dict(), )
    ...
    def restore(self, model_args, training_args):
        ...
        opt_dict, 
        self.spatial_lr_scale,
        *optional_dicts) = model_args

        self.training_setup(training_args)
        if self.dynamic == 'FeatureLine':
            self.feat_line.load_state_dict(optional_dicts[0])
            # self.feat_line_face.load_state_dict(optional_dicts[0])
            # self.feat_line_head.load_state_dict(optional_dicts[1])

        if self.static == 'None':
            self._features_dc = optional_dicts[2]
            self._features_rest = optional_dicts[3]
        elif self.static == 'triplane':
            self.triplane.load_state_dict(optional_dicts[2])
    ...
    def get_xyz_cano(self):
        xyz_cano = torch.bmm(self.face_orien_mat_cano[self.binding], self._xyz[..., None]).squeeze(-1)
        return xyz_cano * self.face_scaling_cano[self.binding] + self.face_center_cano[self.binding]

    def get_features(self, cam_pos = None):
        if self.static == 'None':
        ...
        #transform view direction from deformed space to canonical space

        view_dir_def = cam_pos.to(xyz.device).repeat(xyz.shape[0], 1) - xyz
        view_dir_def = view_dir_def / view_dir_def.norm(dim=1, keepdim=True)

        view_dir_cano = torch.bmm(torch.bmm(self.face_orien_mat_cano[self.binding], self.face_orien_mat[self.binding].transpose(2, 1)), view_dir_def[..., None]).squeeze(-1) * self.face_scaling_cano[self.binding] / self.face_scaling[self.binding]
        view_dir_cano = view_dir_cano / view_dir_cano.norm(dim=1, keepdim=True)

        xyz_cano = self.xyz_cano
        rgb = self.triplane(xyz_cano, view_dir_cano)

        return rgb

    def get_opacity(self):
        if self.dynamic == 'FeatureLine':
            if len(self._opacity_offset) == 0 or not len(self._opacity_offset) == len(self._opacity_cano):
                self._opacity_offset = torch.zeros_like(self._opacity_cano)
            self._opacity = self._opacity_cano + self._opacity_offset.to(self._opacity_cano.device)
        else:
            self._opacity = self._opacity_cano

        return self.opacity_activation(self._opacity)
    
    @property
    def get_opacity_cano(self):
        return self.opacity_activation(self._opacity_cano)
    ...
    def create_from_pcd(self, pcd : Optional[BasicPointCloud], spatial_lr_scale : float):
        ...
        self._opacity_cano = nn.Parameter(opacities.requires_grad_(True))
        # self._opacity_offset = torch.zeros_like(self._opacity_cano)
        self._opacity = self._opacity_cano #+ self._opacity_offset
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [self._opacity_cano], 'lr': training_args.opacity_lr, "name": "opacity_cano"},
            {'params': [self._scaling], 'lr': training_args.scaling_lr, "name": "scaling"},
            {'params': [self._rotation], 'lr': training_args.rotation_lr, "name": "rotation"}
        ]
        if self.static == 'None':
            l += [{'params': [self._features_dc], 'lr': training_args.feature_lr, "name": "f_dc"},
    ...
    def save_ply(self, path):
        ...
        opacities = self._opacity_cano.detach().cpu().numpy()
    ...
    def reset_opacity(self):
        opacities_new = inverse_sigmoid(torch.min(self.get_opacity_cano, torch.ones_like(self.get_opacity_cano)*0.01))
        optimizable_tensors = self.replace_tensor_to_optimizer(opacities_new, "opacity_cano")
        self._opacity_cano = optimizable_tensors["opacity_cano"]
    ...
    def load_ply(self, path, **kwargs):
        ...
        opacities = np.asarray(plydata.elements[0]["opacity_cano"])[..., np.newaxis]
        ...
        self._opacity_cano = nn.Parameter(torch.tensor(opacities, dtype=torch.float, device="cuda").requires_grad_(True))
        ...
        if len(binding_names) > 0:
            ...
            self.binding = torch.tensor(binding, dtype=torch.int32, device="cuda").squeeze(-1).long()
    ...
    def densification_postfix_triplane(self, new_xyz, new_opacities, new_scaling, new_rotation):
        d = {"xyz": new_xyz,
        "opacity_cano": new_opacities, 
        "scaling" : new_scaling,
        ...
        xyz = self.get_xyz
        self.xyz_gradient_accum = torch.zeros((xyz.shape[0], 1), device="cuda")
        self.denom = torch.zeros((xyz.shape[0], 1), device="cuda")
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_opacity = self._opacity_cano[selected_pts_mask].repeat(N,1)
        if self.binding is not None:
            # Toyota Motor Europe NV/SA and its affiliated companies retain all intellectual property and proprietary rights in and to the following code lines and related documentation. Any commercial use, reproduction, disclosure or distribution of these code lines and related documentation without an express license agreement from Toyota Motor Europe NV/SA is strictly prohibited.
        ...
        if self.static == 'triplane':
            self.densification_postfix_triplane(new_xyz, new_opacity, new_scaling, new_rotation)
        else:
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_opacities = self._opacity_cano[selected_pts_mask]
        ...
        if self.static == 'triplane':
            self.densification_postfix_triplane(new_xyz, new_opacities, new_scaling, new_rotation)
        else: 
            self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation)

    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        #I use opacity in canonical space for gaussian splats densification and pruning, I think this pruning strategy can be further explored
        ...
        prune_mask = (self.get_opacity_cano < min_opacity).squeeze()
        if max_screen_size:
