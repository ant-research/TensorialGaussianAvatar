...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    if dataset.bind_to_mesh:
        gaussians = FlameGaussianModel(dataset.source_path, dataset.sh_degree, \
            dataset.disable_flame_static_offset, not_finetune_flame_params = True, \
            static = dataset.static, dynamic = dataset.dynamic)
    ...
    labels = np.load(dataset.source_path + '/train_cluster_16.npz')['labels']
    ...
    prob_lst = torch.tensor(prob_lst).double()
    prob_lst /= torch.sum(prob_lst)

    offset_start_iter = 60000

    sampler = WeightedRandomSampler(weights = prob_lst, num_samples = len(train_cameras), replacement = True)
    loader_camera_train = DataLoader(scene.getTrainCameras(), sampler = sampler, batch_size=None, num_workers=8, pin_memory=True, persistent_workers=True)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if gaussians.binding != None:
            ...
            if gaussians.dynamic != 'None' and iteration > offset_start_iter:
                gaussians.update_offset()
        ...
        if dataset.dynamic == 'FeatureLine' and iteration > offset_start_iter:
            losses['norm'] = gaussians.loss_offset_normalize(viewpoint_cam.timestep) 
        # losses['param'] = gaussians.loss_flame_param_normalize(viewpoint_cam.timestep) * 10
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                ...
                postfix['splats'] = gaussians._xyz.shape[0]
                if 'norm' in losses:
                    postfix['norm'] = f"{losses['norm']:.{5}f}"
                # if 'xyz' in losses:
                #     postfix["xyz"] = f"{losses['xyz']:.{5}f}"
                # if 'scale' in losses:
                #     postfix["scale"] = f"{losses['scale']:.{5}f}"
                # if 'ssim' in losses:
                #     postfix["ssim"] = f"{losses['ssim']:.{5}f}"
                if 'param' in losses:
                    postfix['param'] = f"{losses['param']:.{5}f}"
